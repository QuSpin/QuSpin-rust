...
crates/
  ...
  quspin-nlce/       # Numerical linked-cluster expansion (rectangle + topological bond/site NLCE, ED solver) on quspin-core
python/
...
- **NLCE (`quspin-nlce`).** Trait pipeline `Lattice` → `ClusterGenerator` → `ClusterSolver<M: Model>` → `Property` → `combine`/`run_nlce` → `Resummation`; see `crates/quspin-nlce/README.md`. Generators: `RectangleGenerator`, `TopologicalGenerator<L, K: NodeKind>` (`BondGenerator`, `SiteGenerator`: Redelmeier enumeration over nodes + labelled `canon` graph canonicalisation → topological clusters, any `Lattice`); `ClusterSet` saves/loads a model-independent DAG. Resummation: `Bare`, `Wynn`, `Euler` over `Componentwise` properties. The 4×4 identity test is `#[ignore]`d (run with `--release -- --ignored`).
...
- **Symmetry-group construction (Python).** `SymmetryGroup(n_sites, lhss)` in `quspin_rs.symmetry` is the user-facing handle. Construct elements via `Lattice(perm)`, `Local(perm_vals, locs=None)`, `Composite(perm, perm_vals, locs=None)`. Add via `.add(elem, char)`, `.add_cyclic(elem, k=…|eta=±1|char=…)`, `.close(generators, char=…)`, or `.product(other)`. Pass the result as the first positional argument to `*Basis.symmetric(group, ham, seeds)` — `n_sites` and `lhss` come from the group, `FermionBasis` rejects `lhss != 2`. Spec: `docs/superpowers/specs/2026-04-26-python-symmetry-group-api-design.md`.
